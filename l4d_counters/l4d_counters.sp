...
int g_iKillsCommon[L4D_MAXPLAYERS + 1];
...
Action Cmd_ShowKills(int client, int args)
{
	if (client == 0)
		return Plugin_Handled;

	PrintTopFrags(client, false);
	return Plugin_Handled;
}
...
void Event_PlayerDeath(Event event, const char[] name, bool dontBroadcast)
{
	...
	{
		char sVictim[16];
		event.GetString("victimname", sVictim, sizeof(sVictim));

		if (StrEqual(sVictim, "infected", false))
		{
